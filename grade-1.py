...
def compile(build_dir, cu_file, with_arch_flag=False, sm_version=None):
    compile_command = f"nvcc -Xcompiler -rdynamic -lineinfo -x cu {os.path.basename(cu_file)} -o a.out"
...
def grade_file(cu_file, perf_only=False, with_arch_flag=False, sm_version=None, GPU_arch=None, build_dir=None):
    # Use build_dir if provided, otherwise use current working directory
    if build_dir is None:
        build_dir = os.getcwd()
    try:
        if not compile(build_dir, cu_file, with_arch_flag, sm_version):
            return {
...
if __name__ == "__main__":
    ...
    parser.add_argument("cu_file", nargs="?", help="The .cu file to grade.")
    ...
    if not args.cu_file:
        print("Usage: python grade.py <submission.cu> [--perf-only] [--build-dir <directory>]")
        exit(1)

    result = grade_file(args.cu_file, perf_only=args.perf_only, build_dir=args.build_dir)